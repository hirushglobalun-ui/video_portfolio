"use client";

import React, { useState, useEffect } from "react";
import { Plus, Trash2, FileEdit, X, RefreshCw, Cpu } from "lucide-react";
import AdminHeader from "@/components/admin/AdminHeader";
import ConfirmModal from "@/components/admin/ConfirmModal";
import {
  collection,
  getDocs,
  doc,
  setDoc,
  deleteDoc,
} from "firebase/firestore";
import { db, isFirebaseConfigured } from "@/lib/firebase/client";
import { getSoftware } from "@/lib/data";
import { SoftwareTool } from "@/types/cms";

export default function AdminSoftwarePage() {
  const [software, setSoftware] = useState<SoftwareTool[]>([]);
  const [loading, setLoading] = useState(true);

  // Modal
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingSoftware, setEditingSoftware] = useState<SoftwareTool | null>(null);

  // Form states
  const [name, setName] = useState("");
  const [role, setRole] = useState("");
  const [monogram, setMonogram] = useState("");
  const [description, setDescription] = useState("");
  const [highlightsInput, setHighlightsInput] = useState("");
  const [color, setColor] = useState("from-[#FF4E27]/20 to-[#FFA337]/10");
  const [displayOrder, setDisplayOrder] = useState(1);
  const [active, setActive] = useState(true);

  // Delete modal
  const [deleteTarget, setDeleteTarget] = useState<SoftwareTool | null>(null);
  const [isDeleting, setIsDeleting] = useState(false);

  const loadData = async () => {
    setLoading(true);
    try {
      if (isFirebaseConfigured() && db) {
        const snap = await getDocs(collection(db, "software"));
        if (!snap.empty) {
          setSoftware(
            snap.docs
              .map((d) => ({ id: d.id, ...d.data() } as SoftwareTool))
              .sort((a, b) => (a.displayOrder || 0) - (b.displayOrder || 0))
          );
        } else {
          setSoftware(await getSoftware());
        }
      } else {
        setSoftware(await getSoftware());
      }
    } catch (err) {
      console.error(err);
      setSoftware(await getSoftware());
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    loadData();
  }, []);

  const openCreateModal = () => {
    setEditingSoftware(null);
    setName("");
    setRole("");
    setMonogram("");
    setDescription("");
    setHighlightsInput("");
    setColor("from-[#FF4E27]/20 to-[#FFA337]/10");
    setDisplayOrder(software.length + 1);
    setActive(true);
    setIsModalOpen(true);
  };

  const openEditModal = (sw: SoftwareTool) => {
    setEditingSoftware(sw);
    setName(sw.name);
    setRole(sw.role);
    setMonogram(sw.monogram);
    setDescription(sw.description);
    setHighlightsInput((sw.highlights || []).join(", "));
    setColor(sw.color || "from-[#FF4E27]/20 to-[#FFA337]/10");
    setDisplayOrder(sw.displayOrder || 1);
    setActive(sw.active !== false);
    setIsModalOpen(true);
  };

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) return;

    try {
      const highlightsArray = highlightsInput
        .split(",")
        .map((h) => h.trim())
        .filter(Boolean);

      const softwareData: SoftwareTool = {
        name: name.trim().toUpperCase(),
        role: role.trim().toUpperCase(),
        monogram: monogram.trim() || name.slice(0, 2).toUpperCase(),
        description: description.trim(),
        highlights: highlightsArray,
        color,
        borderHover: "hover:border-[#FF3B1F]/50",
        accentText: "text-[#FF3B1F]",
        displayOrder: Number(displayOrder) || 1,
        active,
      };

      const docId = editingSoftware?.id || `sw-${Date.now()}`;

      if (db) {
        await setDoc(doc(db, "software", docId), softwareData, { merge: true });
      }

      if (editingSoftware) {
        setSoftware((prev) =>
          prev.map((s) => (s.id === editingSoftware.id ? { id: docId, ...softwareData } : s))
        );
      } else {
        setSoftware((prev) => [...prev, { id: docId, ...softwareData }]);
      }

      setIsModalOpen(false);
    } catch (err) {
      console.error(err);
    }
  };

  const handleDelete = async () => {
    if (!deleteTarget) return;
    setIsDeleting(true);
    try {
      if (db && deleteTarget.id) {
        await deleteDoc(doc(db, "software", deleteTarget.id));
      }
      setSoftware((prev) => prev.filter((s) => s.id !== deleteTarget.id));
      setDeleteTarget(null);
    } catch (err) {
      console.error(err);
    } finally {
      setIsDeleting(false);
    }
  };

  return (
    <div className="flex-1 flex flex-col min-h-screen bg-[#F9FAFB]">
      <AdminHeader
        title="Software & Proficiency"
        subtitle="Manage editing environments (DaVinci Resolve, Premiere Pro, After Effects)"
        onOpenMobile={() => {}}
        action={
          <div className="flex items-center gap-2">
            <button
              onClick={loadData}
              disabled={loading}
              className="p-2 rounded-xl text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors"
              title="Refresh"
            >
              <RefreshCw className={`w-4 h-4 ${loading ? "animate-spin" : ""}`} />
            </button>
            <button
              onClick={openCreateModal}
              className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-[#FF3B1F] hover:bg-[#E0341A] text-white font-mono text-xs font-bold uppercase tracking-wider transition-all shadow-md shadow-[#FF3B1F]/20 cursor-pointer"
            >
              <Plus className="w-4 h-4" />
              <span>Add Software</span>
            </button>
          </div>
        }
      />

      <main className="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl w-full mx-auto">
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-xs">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="border-b border-gray-200 bg-gray-50 text-gray-500 font-mono uppercase tracking-wider text-[10px]">
                <tr>
                  <th className="p-4 w-12 text-center">#</th>
                  <th className="p-4">Monogram</th>
                  <th className="p-4">Software Name</th>
                  <th className="p-4">Role / Focus</th>
                  <th className="p-4">Highlights / Modules</th>
                  <th className="p-4">Status</th>
                  <th className="p-4 text-right">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 font-sans">
                {software.map((sw, idx) => (
                  <tr
                    key={sw.id || idx}
                    className="hover:bg-gray-50/80 transition-colors group"
                  >
                    <td className="p-4 text-center font-mono text-gray-400">
                      {sw.displayOrder || idx + 1}
                    </td>
                    <td className="p-4">
                      <div className="w-9 h-9 rounded-lg bg-gray-100 border border-gray-200 flex items-center justify-center font-mono font-bold text-gray-900 text-xs">
                        {sw.monogram}
                      </div>
                    </td>
                    <td className="p-4 font-bold text-gray-900 tracking-wide group-hover:text-[#FF3B1F] transition-colors">
                      {sw.name}
                    </td>
                    <td className="p-4 font-mono text-xs text-[#FF3B1F] font-semibold">
                      {sw.role}
                    </td>
                    <td className="p-4">
                      <div className="flex flex-wrap gap-1 max-w-sm">
                        {(sw.highlights || []).map((h, hIdx) => (
                          <span
                            key={hIdx}
                            className="px-2 py-0.5 rounded bg-gray-100 border border-gray-200 text-[10px] font-mono text-gray-600"
                          >
                            {h}
                          </span>
                        ))}
                      </div>
                    </td>
                    <td className="p-4">
                      <span
                        className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-mono uppercase font-bold ${
                          sw.active !== false
                            ? "bg-emerald-50 text-emerald-700 border border-emerald-200"
                            : "bg-gray-100 text-gray-500 border border-gray-200"
                        }`}
                      >
                        {sw.active !== false ? "Active" : "Hidden"}
                      </span>
                    </td>
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          onClick={() => openEditModal(sw)}
                          className="p-1.5 rounded-lg text-gray-400 hover:text-[#FF3B1F] hover:bg-[#FF3B1F]/10 transition-colors cursor-pointer"
                          title="Edit"
                        >
                          <FileEdit className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => setDeleteTarget(sw)}
                          className="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors cursor-pointer"
                          title="Delete"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      </main>

      {/* Modal */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs">
          <div className="relative w-full max-w-md bg-white border border-gray-200 rounded-2xl p-6 shadow-2xl space-y-5">
            <div className="flex items-center justify-between border-b border-gray-100 pb-4">
              <h3 className="text-base font-bold font-display text-gray-900 uppercase">
                {editingSoftware ? "Edit Software" : "Add Software Tool"}
              </h3>
              <button
                onClick={() => setIsModalOpen(false)}
                className="p-1 rounded-lg text-gray-400 hover:text-gray-700 hover:bg-gray-100 cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            <form onSubmit={handleSave} className="space-y-4">
              <div className="grid grid-cols-3 gap-3">
                <div className="space-y-1">
                  <label className="text-xs font-mono uppercase text-gray-700 font-semibold">
                    Monogram
                  </label>
                  <input
                    type="text"
                    value={monogram}
                    onChange={(e) => setMonogram(e.target.value)}
                    placeholder="DR"
                    maxLength={3}
                    required
                    className="w-full h-10 px-3 bg-gray-50 border border-gray-300 rounded-xl text-xs font-mono text-center font-bold text-gray-900 focus:outline-none focus:bg-white focus:border-[#FF3B1F] focus:ring-1 focus:ring-[#FF3B1F]"
                  />
                </div>
                <div className="col-span-2 space-y-1">
                  <label className="text-xs font-mono uppercase text-gray-700 font-semibold">
                    Tool Name *
                  </label>
                  <input
                    type="text"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    placeholder="DAVINCI RESOLVE"
                    required
                    className="w-full h-10 px-3 bg-gray-50 border border-gray-300 rounded-xl text-xs text-gray-900 focus:outline-none focus:bg-white focus:border-[#FF3B1F] focus:ring-1 focus:ring-[#FF3B1F]"
                  />
                </div>
              </div>

              <div className="space-y-1">
                <label className="text-xs font-mono uppercase text-gray-700 font-semibold">
                  Role / Focus *
                </label>
                <input
                  type="text"
                  value={role}
                  onChange={(e) => setRole(e.target.value)}
                  placeholder="COLOR GRADING & FINISHING"
                  required
                  className="w-full h-10 px-3 bg-gray-50 border border-gray-300 rounded-xl text-xs text-gray-900 focus:outline-none focus:bg-white focus:border-[#FF3B1F] focus:ring-1 focus:ring-[#FF3B1F]"
                />
              </div>

              <div className="space-y-1">
                <label className="text-xs font-mono uppercase text-gray-700 font-semibold">
                  Description
                </label>
                <textarea
                  rows={2}
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  placeholder="Primary post-production environment for..."
                  className="w-full p-3 bg-gray-50 border border-gray-300 rounded-xl text-xs text-gray-900 focus:outline-none focus:bg-white focus:border-[#FF3B1F] focus:ring-1 focus:ring-[#FF3B1F]"
                />
              </div>

              <div className="space-y-1">
                <label className="text-xs font-mono uppercase text-gray-700 font-semibold">
                  Highlights & Skills (comma separated)
                </label>
                <input
                  type="text"
                  value={highlightsInput}
                  onChange={(e) => setHighlightsInput(e.target.value)}
                  placeholder="Color Grading, Custom LUTs, Fusion VFX, Fairlight"
                  className="w-full h-10 px-3 bg-gray-50 border border-gray-300 rounded-xl text-xs text-gray-900 focus:outline-none focus:bg-white focus:border-[#FF3B1F] focus:ring-1 focus:ring-[#FF3B1F]"
                />
              </div>

              <div className="grid grid-cols-2 gap-3 items-center pt-2">
                <div className="space-y-1">
                  <label className="text-xs font-mono uppercase text-gray-700 font-semibold">
                    Display Order
                  </label>
                  <input
                    type="number"
                    value={displayOrder}
                    onChange={(e) => setDisplayOrder(Number(e.target.value))}
                    className="w-full h-10 px-3 bg-gray-50 border border-gray-300 rounded-xl text-xs text-gray-900 font-mono focus:outline-none focus:bg-white focus:border-[#FF3B1F]"
                  />
                </div>

                <div className="flex items-center gap-2 pt-5">
                  <input
                    type="checkbox"
                    id="swActiveToggle"
                    checked={active}
                    onChange={(e) => setActive(e.target.checked)}
                    className="accent-[#FF3B1F] w-4 h-4 cursor-pointer"
                  />
                  <label
                    htmlFor="swActiveToggle"
                    className="text-xs font-mono uppercase text-gray-700 font-semibold cursor-pointer"
                  >
                    Active
                  </label>
                </div>
              </div>

              <div className="flex items-center justify-end gap-2 pt-4 border-t border-gray-100">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="px-4 py-2 rounded-xl text-xs font-mono uppercase text-gray-500 hover:text-gray-800 cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="px-5 py-2 rounded-xl bg-[#FF3B1F] hover:bg-[#E0341A] text-white font-mono text-xs font-bold uppercase tracking-wider transition-all shadow-md shadow-[#FF3B1F]/20 cursor-pointer"
                >
                  Save Software
                </button>
              </div>
            </form>
          </div>
        </div>
      )}

      <ConfirmModal
        isOpen={Boolean(deleteTarget)}
        title="Delete Software"
        message={`Are you sure you want to delete "${deleteTarget?.name}"?`}
        isLoading={isDeleting}
        onConfirm={handleDelete}
        onClose={() => setDeleteTarget(null)}
      />
    </div>
  );
}
